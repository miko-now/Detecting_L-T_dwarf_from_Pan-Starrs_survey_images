...
def verify_image_label(args):
    """Verify one image-label pair (.jpg/.png/.webp/... or .npy) and its YOLO txt label."""
    ...
    try:
        ...
        if im_file.lower().endswith(".npy"):
            ...
            if not isinstance(arr, np.ndarray):
                raise TypeError(f"{im_file} does not contain a numpy array")

            if arr.ndim == 2:
                # 灰度 HxW
                h, w = arr.shape
            elif arr.ndim == 3:
                # 既支持 HWC 也支持 CHW。经验规则：某一维 <= 6 视作通道维
                if arr.shape[0] <= 6 and arr.shape[1] >= 10 and arr.shape[2] >= 10:
                    # CHW
                    c, h, w = arr.shape
                elif arr.shape[-1] <= 6 and arr.shape[0] >= 10 and arr.shape[1] >= 10:
                    # HWC
                    h, w, c = arr.shape
                else:
                    # 兜底：取最大的两维作为 H、W（不推荐，但能避免误判直接报错）
                    dims = sorted(arr.shape)
                    h, w = dims[-2], dims[-1]
            else:
                raise ValueError(f"{im_file} must be HxW or HxWxC/CHW, got shape {arr.shape}")
            ...
            fmt = im.format.lower() if hasattr(im, "format") and im.format else ""
            # 注意：npy 不走这里，其他图片才检查格式
            assert (fmt in IMG_FORMATS) or (fmt == ""), f"invalid image format {fmt}. {FORMATS_HELP_MSG}"
        ...
        if os.path.isfile(lb_file):
            ...
            # seg 情况（检测任务：一行>6个数，表示多边形）
            if rows and (not keypoint) and any(len(x) > 6 for x in rows):
                classes = np.array([x[0] for x in rows], dtype=np.float32)
                segments = [np.array(x[1:], dtype=np.float32).reshape(-1, 2) for x in rows]  # (cls, xy1...)
                lb = np.concatenate((classes.reshape(-1, 1), segments2boxes(segments)), 1)  # (cls, xywh)
            else:
                lb = np.array(rows, dtype=np.float32) if rows else np.zeros((0, 5), dtype=np.float32)
            ...
            if nl:
                ...
                # 归一化范围/非负
                assert points.max() <= 1, f"non-normalized or out-of-bounds coords {points[points > 1]}"
                assert lb.min() >= 0, f"negative label values {lb[lb < 0]}"

                # 类别越界 & 去重
                max_cls = lb[:, 0].max() if nl else -1
                assert max_cls <= num_cls - 1, (
            ...
            nm = 1  # label missing
            # 这里修复了原代码的变量名 bug：keypoints -> keypoint
            lb = np.zeros((0, 5 + nkpt * ndim if keypoint else 5), dtype=np.float32)
        ...
        # 训练缓存里只保留前 5 列（cls+xywh）
        lb = lb[:, :5] if lb.size else lb
