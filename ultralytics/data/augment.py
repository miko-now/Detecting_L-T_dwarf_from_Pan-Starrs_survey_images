...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        color=(114, 114, 114)
        import numpy as np
        if img.ndim == 3 and img.shape[2] > 3:  # 多通道 (>3)，OpenCV 不支持
...
class Format:
    ...
    def _format_img(self, img):
        ...
        # -------- 逐通道 Min-Max 归一化 --------
        # 对每个通道分别算 min 和 max
        # img_min = img.min(axis=(1, 2), keepdims=True)  # (C, 1, 1)
        # img_max = img.max(axis=(1, 2), keepdims=True)  # (C, 1, 1)
        #
        # denom = img_max - img_min
        # # 避免除以 0：如果某通道 max==min，就整通道置 0
        # img = np.where(denom > 0, (img - img_min) / denom, 0.0)
        # # 转为 torch.Tensor
        ...
        # -------- 使用全局各波段 min/max 做归一化 --------
        # 给定每个波段的 25% 最小值和 75% 最大值（以 g, r, i, z 为例）
        #75+5
        # band_min_5_percent = {
        #     'g': -944,
        #     'r': -1520,
        ...
        # }
        # for c in range(5):
        #     band_name = ['g', 'r', 'i', 'z', 'y'][c]  # g, r, i, z, y 的顺序
        #     min_5_percent = band_min_5_percent[band_name]
        #     max_75_percent = band_max_75_percent[band_name]
        #     # 计算归一化分母
        #     denom = max_75_percent - min_5_percent
        #     # 对每个通道进行归一化
        #     if denom > 0:
        #         img[c, ...] = (img[c, ...] - min_5_percent) / denom
        #     else:
        #         img[c, ...] = np.zeros_like(img[c, ...], dtype=np.float32)
        #     # 将像素值限制在 [0, 1] 范围内
        # img = np.clip(img, 0.0, 1.0)
        # # 转为 torch.Tensor 类型
        # img = torch.from_numpy(img.astype(np.float32))
