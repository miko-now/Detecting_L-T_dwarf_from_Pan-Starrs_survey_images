            ...
            if self.video_flag[self.count]:
                ...
                if path.split(".")[-1].lower() == "heic":
                    ...
                    if path.endswith('.npy'):
                        ...
                        # 判断数据维度
                        if im0.ndim == 2:  # 灰度图
                            im0 = np.stack([im0] * 3, axis=-1)  # (H, W) → (H, W, 3)
                        elif im0.ndim == 3:
                            if im0.shape[0] in (3, 4, 5):  # (C, H, W)
                                im0 = np.nan_to_num(im0, nan=0.0)
                                GLOBAL_MIN = -6425
                                GLOBAL_MAX = 1450150
                                denom = GLOBAL_MAX - GLOBAL_MIN
                                if denom > 0:
                                    im0 = (im0 - GLOBAL_MIN) / denom
                                else:
                                    im0 = np.zeros_like(im0, dtype=np.float32)
                                im0 = np.clip(im0, 0.0, 1.0)

                                im0 = im0.transpose(1, 2, 0)  # → (H, W, C)
                            elif im0.shape[2] in (3, 4, 5):  # 已经是 (H, W, C)
                                pass
                            else:
                                raise ValueError(f"Unsupported npy shape: {im0.shape}")
                        im0 = np.ascontiguousarray(im0.astype(np.float32))
                    else:
                        im0 = imread(path)  # BGR
                if im0 is None:
