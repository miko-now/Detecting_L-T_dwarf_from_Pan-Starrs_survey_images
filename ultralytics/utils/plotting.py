...
@threaded
def plot_images(
        images: Union[torch.Tensor, np.ndarray, List[Union[torch.Tensor, np.ndarray]]],
    ...
    NOTE: 为了可视化稳定，这里把任意通道数的输入都转换成 3 通道（仅用于画图，不影响训练）
    """
    import math
    ...
    def ensure_chw3(im: Union[np.ndarray, torch.Tensor]) -> np.ndarray:
        """
        把任意 HWC/CHW、任意通道数的图像，转成 CHW 且 3 通道（可视化专用）.
        规则：
          - HWC 或 CHW 都能识别
          - C==1 重复到 3 通道
          - C>3 截取前 3 通道
          - 如果像素值范围<=1，按 255 放大到 uint8
        """
        im = to_numpy(im)
        assert im.ndim in (2, 3), f"Expected 2D/3D image for plotting, got shape {im.shape}"

        # 扩维到 CHW
        if im.ndim == 2:  # H,W -> 1,H,W
            im = im[None, ...]
        if im.ndim == 3:
            h, w = im.shape[0], im.shape[1]
            # 判断是 CHW 还是 HWC：如果第一个维度很小(<=6)且后两维像图像大小，则认为是 CHW，否则认为是 HWC
            if im.shape[0] <= 6 and im.shape[1] >= 10 and im.shape[2] >= 10:
                chw = im
            elif im.shape[2] <= 6 and im.shape[0] >= 10 and im.shape[1] >= 10:
                chw = im.transpose(2, 0, 1)  # HWC -> CHW
            else:
                # 不太确定就尽量当 HWC 处理
                chw = im.transpose(2, 0, 1)

        C = chw.shape[0]
        if C == 1:
            chw = np.repeat(chw, 3, axis=0)  # 灰度 -> 3 通道
        elif C < 3:
            # 不足 3 通道用 0 补足
            pad = np.zeros((3 - C, *chw.shape[1:]), dtype=chw.dtype)
            chw = np.concatenate([chw, pad], axis=0)
        elif C > 3:
            # 取riz通道
            image_r = chw[3, ...]  # z 通道 -> 红色
            image_g = chw[2, ...]  # i 通道 -> 绿色
            image_b = chw[1, ...]  # r 通道 -> 蓝色
            rgb = make_lupton_rgb(image_r, image_g, image_b, stretch=3, Q=1)
            chw = rgb.transpose(2, 0, 1)  # HWC -> CHW
            # chw = chw[1:4, ...]  # 多于 3 通道 -> 取前 3

        # 归一化到 0~255 的 uint8 便于画图
        if chw.dtype != np.uint8:
            if np.max(chw) <= 1.0:
                chw = (chw * 255.0).clip(0, 255)
    ...
    # ---- 兼容 list 或 Tensor/ndarray 批次 ----
    if isinstance(images, list):
        images = [ensure_chw3(im) for im in images]
        images = np.stack(images, axis=0)  # (B,3,H,W)
    else:
        images = to_numpy(images)  # (B,C,H,W) 但可能 C!=3
        # 对 batch 内每张裁/扩到 3 通道
        images = np.stack([ensure_chw3(im) for im in images], axis=0)  # (B,3,H,W)

    bs, _, h, w = images.shape  # batch size, _, height, width
    bs = int(min(bs, max_subplots))  # 限制最多可视化张数
    ns = int(np.ceil(bs ** 0.5))  # 子图数量（正方形矩阵）

    # ---- 构建 mosaic 画布 ----
    mosaic = np.full((int(ns * h), int(ns * w), 3), 255, dtype=np.uint8)  # 白底
    for i in range(bs):
        x, y = int(w * (i // ns)), int(h * (i % ns))  # block origin
        mosaic[y: y + h, x: x + w, :] = images[i].transpose(1, 2, 0)  # CHW->HWC

    # ---- 可选：整体缩放到不超过 max_size ----
    ...
    txt_name = Path(fname).with_suffix('.txt')
        ...
        for i in range(bs):
            ...
            if len(cls) > 0:
                ...
                if len(bboxes):
                    ...
                    if len(boxes):
                        if boxes[:, :4].max() <= 1.1:  # 归一化坐标
                            boxes[..., [0, 2]] *= w
                            boxes[..., [1, 3]] *= h
                        elif scale < 1:  # 绝对坐标在 mosaic 缩放后也要缩放
                            boxes[..., :4] *= scale
                    boxes[..., 0] += x
                    boxes[..., 1] += y
                    ...
                    for j, box in enumerate(boxes.astype(np.int64).tolist()):
                        ...
                        if labels or conf[j] > conf_thres:
                            ...
                            file.write(f"{Path(paths[i]).name}, {label}, {box}\n")
            ...
            if len(kpts):
                ...
                for j in range(len(kpts_)):
                    if labels or (confs is not None and confs[idx][j] > conf_thres):
